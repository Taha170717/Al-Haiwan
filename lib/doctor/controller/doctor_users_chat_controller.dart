      ...
      _db
        ...
        for (var doc in snapshot.docs) {
          try {
            ...
            // compute a robust display name from common fields
            String rawName = '';
            try {
              rawName = (userData['username'] ?? userData['name'] ?? userData['displayName'] ?? userData['fullName'] ?? (userData['basicInfo'] != null ? userData['basicInfo']['fullName'] : null) ?? userData['ownerName'] ?? userData['email'] ?? '').toString();
            } catch (_) {
              rawName = '';
            }
            final displayName = rawName.trim().isNotEmpty ? rawName : (userData['email'] ?? userDoc.id ?? 'Unknown User').toString();
