...
class _AdminOrderManagementScreenState extends State<AdminOrderManagementScreen>
  ...
  void initState() {
    ...
    // Clear previous state and load orders once after the first frame so
    // the UI shows data immediately when opening this screen.
  ...
  Future<void> _loadOrders({bool forceRefresh = false}) async {
    ...
    try {
      ...
      if (mounted) {
        CustomSnackbar.showError('Failed to load orders', 'Please check your connection', context: context, duration: const Duration(seconds: 3));
      }
    }
  }
  ...
  Widget _buildStatusTab(String title, String status, double screenWidth, bool isWeb) {
    return Obx(() {
      ...
      // compute status counts reactively
      final total = orderController.orders.length;
      final count = status.isEmpty
          ? total
          : orderController.orders.where((o) => o.status == status).length;
      ...
      return GestureDetector(
        ...
        child: Container(
          ...
          padding: EdgeInsets.symmetric(
            horizontal: isWeb ? 20 : screenWidth * 0.03,
            vertical: isWeb ? 12 : screenWidth * 0.02,
          ),
          decoration: BoxDecoration(
          ...
          child: Row(
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: isSelected ? Colors.white24 : const Color(0xFFF3F4F6),
                  borderRadius: BorderRadius.circular(12),
                ),
                child: Text(
                  count.toString(),
  ...
  void _updateOrderStatusDirectly(String orderId, String newStatus) async {
    ...
    if (success) {
      CustomSnackbar.showSuccess('Order updated', 'Order status updated successfully!', context: context, duration: const Duration(seconds: 2));
    } else {
      CustomSnackbar.showError('Order update failed', 'Failed to update order status. Please try again.', context: context, duration: const Duration(seconds: 3));
    }
