...
class CartViewModel extends GetxController {
  ...
  Future<void> _checkAndRemoveExpiredItems() async {
    ...
    for (final item in expiredItems) {
      ...
      CustomSnackbar.showAccent('Item Expired', '${item.name} was removed from cart (30 min timeout)', duration: const Duration(seconds: 3));
    }
  }
  ...
  Future<void> addToCart(String productId, String name, String image,
    ...
    if (productId.isEmpty || productId.trim().isEmpty) {
      CustomSnackbar.showAccent('Error', 'Invalid product. Please try again.', duration: const Duration(seconds: 3));
      return;
    }
    ...
    if (existingIndex != -1) {
      ...
      CustomSnackbar.showAccent('Updated Cart', 'Increased quantity of $name', duration: const Duration(seconds: 2));
    } else {
      ...
      // Use a light snackbar for add-to-cart to keep it subtle
      CustomSnackbar.showLight('Added to Cart', '$name has been added to your cart', duration: const Duration(seconds: 2));
    }
  }
  ...
  Future<void> removeItem(int index) async {
    if (index < cartItems.length) {
      ...
      CustomSnackbar.showAccent('Removed from Cart', '${item.name} has been removed from your cart', duration: const Duration(seconds: 2));
    }
  }
  ...
  Future<void> clearCart() async {
    ...
    CustomSnackbar.showAccent('Cart Cleared', 'All items have been removed from your cart', duration: const Duration(seconds: 2));
  }
