...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void _placeOrder() async {
    ...
    if (nameController.text.trim().isEmpty ||
      ...
      // Use local ScaffoldMessenger to avoid Overlay errors
      try {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please fill in all delivery details', style: TextStyle(color: Colors.white)),
            backgroundColor: Color(0xFFF44336),
            duration: Duration(seconds: 3),
          ),
        );
      } catch (e) {
        // Fallback to Get.snackbar if available
        try {
          Get.snackbar('Missing Information', 'Please fill in all delivery details');
        } catch (_) {
          print('Missing delivery details - user notification suppressed');
        }
      }
      return;
    }
    ...
    if (orderItems.isEmpty) {
      try {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('No valid items found in cart. Please add items and try again.', style: TextStyle(color: Colors.white)),
            backgroundColor: Color(0xFFF44336),
        ...
        try {
          Get.snackbar('Cart Error', 'No valid items found in cart. Please add items and try again.');
        } catch (_) {
          print('Cart error - user notification suppressed');
        }
      }
      return;
    }
    ...
    bool success = false;
    try {
      success = await orderController.createOrder(
        items: orderItems,
      ...
      // Catch any unexpected exceptions from createOrder and show a safe message
      try {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to place order: ${e.toString()}', style: const TextStyle(color: Colors.white)),
            backgroundColor: const Color(0xFFF44336),
            duration: const Duration(seconds: 4),
          ),
        );
      } catch (_) {
        try { Get.snackbar('Error', 'Failed to place order'); } catch (_) { print('Order failed - user notification suppressed'); }
      }
      success = false;
    }
    ...
    if (success) {
      ...
      try {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Your order has been placed successfully. You will receive a confirmation call shortly.', style: TextStyle(color: Colors.white)),
            backgroundColor: Color(0xFF4CAF50),
            duration: Duration(seconds: 5),
          ),
        );
      } catch (e) {
        try { Get.snackbar('Order Placed!', 'Your order has been placed successfully. You will receive a confirmation call shortly.'); } catch (_) { print('Order placed - user notification suppressed'); }
      }
