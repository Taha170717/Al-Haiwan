...
class ProfileController extends GetxController {
  ...
  // Upload configuration (previously ImageKit)
  final String _uploadPublicKey = ImageKitConfig.publicKey;
  final String _uploadPrivateKey = ImageKitConfig.privateKey;
  final String _uploadEndpoint = ImageKitConfig.uploadEndpoint;
  ...
  void onInit() {
    super.onInit();
    loadUserProfile();
    // testImageKitConnection(); // removed test call
  }
  ...
  Future<void> loadUserProfile() async {
    try {
      ...
      // Use project's CustomSnackbar for error display
      CustomSnackbar.showError('Error', 'Failed to load profile: $e');
    } finally {
      isLoading.value = false;
    }
  }

  Future<void> updateUserName(String newName, {bool showSnackbars = true}) async {
    try {
      ...
      if (user != null && newName.trim().isNotEmpty) {
        ...
        if (showSnackbars) CustomSnackbar.showSuccess('Success', 'Profile name updated successfully');
      }
    } catch (e) {
      if (showSnackbars) {
        CustomSnackbar.showError('Error', 'Failed to update profile: $e');
      } else {
        rethrow;
      }
    } finally {
      ...
      if (user != null) {
        ...
        if (currentUser.value != null) {
          currentUser.value = currentUser.value!.copyWith(
            ...
            zipCode: zipCode?.trim().isNotEmpty == true ? zipCode!.trim() : null,
            updatedAt: DateTime.now(),
          );
        }

        if (showSnackbars) CustomSnackbar.showSuccess('Success', 'Profile updated successfully');
      }
    } catch (e) {
      if (showSnackbars) {
        CustomSnackbar.showError('Error', 'Failed to update profile: $e');
  ...
  Future<void> uploadProfileImage({bool showSnackbars = true}) async {
    try {
      isUpdating.value = true;
      ...
      if (kIsWeb) {
        ...
        if (result != null && result.files.isNotEmpty) {
          ...
          if (user != null && file.bytes != null) {
            ...
            final url = await _uploadImageToServer(file.bytes!, file.name);
            if (url != null) {
              ...
              if (showSnackbars) CustomSnackbar.showSuccess('Success', 'Profile image updated successfully');
            } else {
        ...
        if (image != null) {
          ...
          if (user != null) {
            ...
            final url = await _uploadImageToServer(bytes, image.name);
            if (url != null) {
              print('Upload successful, updating Firestore with URL: $url');
      ...
      if (showSnackbars) {
        CustomSnackbar.showError('Error', 'Failed to upload image: $e');
      } else {
        rethrow;
      }
    } finally {
  ...
  Future<String?> _uploadImageToServer(Uint8List bytes, String filename) async {
    try {
      final user = _auth.currentUser;
      if (user == null) return null;

      // Validate credentials
      if (_uploadPublicKey == 'your_public_key_here' ||
          _uploadPrivateKey == 'your_private_key_here') {
        throw Exception('Upload credentials not configured. Please update configuration.');
      }

      // Validate file size (5MB limit)
      const maxFileSize = 5 * 1024 * 1024; // 5MB
      if (bytes.length > maxFileSize) {
        throw Exception('Image file is too large. Maximum size allowed is 5MB.');
      }

      // Validate file type (check if it's actually an image)
      if (!_isValidImageFile(bytes)) {
        throw Exception('Invalid image file format. Please select a valid image (JPEG, PNG, WebP, etc.).');
      }
      ...
      final authString = base64Encode(utf8.encode('$_uploadPrivateKey:'));
      ...
      // Send request with detailed logging (neutral wording)
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          errorMessage = responseBody.isNotEmpty ? responseBody : 'Unknown error occurred';
        }
        ...
        switch (response.statusCode) {
          case 400:
            userMessage = 'Invalid image file or upload parameters. Please try with a different image.';
            break;
          case 401:
            userMessage = 'Authentication failed. Please check upload configuration.';
            break;
          case 403:
            userMessage = 'Upload not allowed. Please check upload permissions.';
            break;
          case 413:
      ...
      print('Error in _uploadImageToServer: $e');
  ...
  // Upload document
  Future<void> uploadDocument({bool showSnackbars = true}) async {
    try {
      isUpdating.value = true;
      FilePickerResult? result = await _filePicker.pickFiles(
      ...
      if (result == null || result.files.isEmpty) {
        print('No document selected or picker cancelled');
        return;
      }
      ...
      if (showSnackbars) CustomSnackbar.showSuccess('Success', 'Document uploaded successfully');
    } catch (e) {
      if (showSnackbars) {
        CustomSnackbar.showError('Error', 'Failed to upload document: $e');
      } else {
        rethrow;
      }
    } finally {
  ...
  // Delete document
  Future<void> deleteDocument(String documentUrl, {bool showSnackbars = true}) async {
    try {
      isUpdating.value = true;
      final user = _auth.currentUser;
      if (user != null) {
        ...
        if (showSnackbars) CustomSnackbar.showSuccess('Success', 'Document deleted successfully');
      }
    } catch (e) {
      if (showSnackbars) {
        CustomSnackbar.showError('Error', 'Failed to delete document: $e');
      } else {
        rethrow;
      }
    } finally {
  ...
  Future<void> signOut({bool showSnackbars = true}) async {
    try {
      await _auth.signOut();
      ...
      if (showSnackbars) CustomSnackbar.showSuccess('Success', 'Signed out successfully');
    } catch (e) {
      if (showSnackbars) {
        CustomSnackbar.showError('Error', 'Failed to sign out: $e');
      } else {
        rethrow;
      }
    }
